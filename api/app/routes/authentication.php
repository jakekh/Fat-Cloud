...
$app->post('/signup', function() use ($database){
	...
	try{
		...
		if (empty($data->email)){
			...
			$httpStatus = 400;
			throw new Exception('The "phone" feild is empty.');
		}
		...
		//get emails that match input email
		...
		$mail->addAddress($data->email, $data->name);         // Name is optional
		$mail->addReplyTo($data->email, $data->name);

		$mail->isHTML(true);                                  // Set email format to HTML

		$mail->Subject = 'Fat Cloud - Signup Confirmation';

		$mail->Body    = 'Please click the link below to confirm your account: <br><br><a href="http://localhost/school/fatcloud/#!/confirm/' . $data->email . '/' . $hash . '">Confirm</a>';
		$mail->AltBody = 'Please click the link below to confirm your account: <a href="http://localhost/school/fatcloud/#!/confirm/' . $data->email . '/' . $hash . '">Confirm</a>';
		...
		if(!$mail->send()) {
		    ...
		    $appResponse->SetStatus(200, "Sign up sent, check your email.");
...
$app->post('/login', function() use ($database){
	...
	try{
		...
		if ($user && password_verify($data->password, $user[0]['password'])){
			...
			$_SESSION['email']    = UnescapeHtml($user[0]['email']);
